require([
    ...
    /* Uncommnet if popup required for iOS
    if (os == 'Android' || os == 'iOS') {
        var template2 = Handlebars.templates['overlayPlayButton'];
        $('#wrapper').append(template2());
        $('.mobile-notify').delay(600).fadeIn(300);
        $('.overlayOkButton, .overlayPlayButton').on('click', function() {
            //Adapt.trigger("navigationView : AudioButton");
            $('.overlayPlayButton').hide();
            $('.mobile-notify').hide();
            Adapt.audio.playAudio();
            //Adapt.trigger('boxMenu:playAudio');
        });
    } else {
        var template = Handlebars.templates['loading'];
        $('#wrapper').append(template());
